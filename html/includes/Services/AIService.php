...
Now, generate the full report following these instructions precisely.";
        ...
        if (isset($response['candidates'][0]['content']['parts'][0]['text'])) {
            return [
                ...
                'usage' => $response['usageMetadata'] ?? ['totalTokenCount' => 0],
                'intermediate_summary' => $searchContext . '<br/>' . implode("\n", $searchSummaryForUI)
            ];
        }
    ...
    public function askGemini($prompt)
    {
        ...
        $this->makeApiCall($this->geminiApiUrl, $prompt);
    }

    // PREFERRED - WORKS!
    public function ask($prompt)
    {
        // 🎯 1. Wrap the string into the 'contents' structure Gemini requires
        $data = [
            'contents' => [
                [
                    'parts' => [
        ...
        // 🎯 2. Execute the call and CAPTURE the result
        $response = $this->makeApiCall($this->geminiApiUrl, $data);

        // 🎯 3. Dig through the response object to find the text
        if (isset($response['candidates'][0]['content']['parts'][0]['text'])) {
            return $response['candidates'][0]['content']['parts'][0]['text'];
        }

        // 🎯 4. Log failure if the structure is missing
        $this->log("Gemini API Error or Empty Response: " . print_r($response, true));
        return "[ SIGNAL_LOST ]";
    }
