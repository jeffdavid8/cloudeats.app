      ...
      <div class="verse result" title="<?= $verse['reference_friendly'] ?>" data-key="<?= $verse['reference_friendly']; ?>">
         ...
         foreach ($verse['data'] as $chapter => $verses)
         {
            ...
            <div class="verse_wrapper">
               ...
               foreach ($verses as $v)
               {
                  $book = $bible->booksFullnames[$v['b']-1];
                  ...
                  $book_number = $bible->addZeros($v['b'], 2);
                  $chapter = $bible->addZeros($v['c'], 3);
                  $verse_number = $bible->addZeros($v['v'], 3);
                  $verse_id = $book_number . $chapter . $verse_number;
                  $v['reference'] = BibleJson::getInstance()->utils->friendlyVerseId($verse_id);
