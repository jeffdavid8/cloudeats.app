   ...
   <div class="keyword result" data-key="<?= $result['keyword'] ?>">
      ...
      <div class="scriptures col s12">
         ...
         foreach ($result['data'] as $chapter => $verses)
         {
            ...
            {
               $book = $bible->booksFullnames[$verse['b']-1];
               ...
               $book_number = $bible->addZeros($verse['b'], 2);
               $chapter = $bible->addZeros($verse['c'], 3);
               $verse_number = $bible->addZeros($verse['v'], 3);
               $verse_id = $book_number . $chapter . $verse_number;
               $verse['reference'] = BibleSql::getInstance()->utils->friendlyVerseId($verse_id);
