...
<!-- Tribute Banner for Local Anglers & Seafood Lovers -->
<div style="background-color: #e0f7fa; border-left: 4px solid #00838f; border-radius: 6px; padding: 12px 16px; margin-bottom: 20px;">
    <p style="margin: 0; font-size: 0.95rem; font-weight: 500; color: #006064;">
        🎣 <strong>To all our local anglers & seafood lovers:</strong> Whether you're reeling 'em in on the water or reeling in dinner here, you're our favorite catch! Customize your order below. 🐟🌊
    </p>
</div>
...
<form id="custom-builder-form" data-product-id="<?php echo intval($productId); ?>">
    <?php foreach ($steps as $step): ?>
        ...
        <div class="builder-step-section" style="margin-bottom: 24px;">
            <h6 style="font-weight: 600; font-size: 1.15rem; color: #00838f;">
            ...
            <?php foreach ($step['options'] as $idx => $opt): 
                ...
                if ($isWidget): 
                    $priceLabel = ($optPrice > 0) ? ' <span style="color: #00838f; font-weight: 500;">(+$' . number_format($optPrice, 2) . ' ea)</span>' : '';
            ?>
            ...
            <?php else: 
                ...
                if ($isRadio) {
                    if ($includedRadioIdx !== null) {
                        $isChecked = ($idx === $includedRadioIdx); // Pre-select the included radio option
                    } else {
                        $isChecked = ($idx === 0 && !empty($step['required'])); // Fallback to first option if required
                    }
                } else {
                    $isChecked = $isIncluded; // Pre-check included checkboxes
                }
                ...
                if ($isIncluded) {
                    ...
                    $priceLabel = ' <span style="color: #00838f; font-weight: 500;">(+$' . number_format($optPrice, 2) . ')</span>';
                } elseif ($optPrice < 0) {
        ...
        <hr style="border: 0; border-top: 2px dashed #b2ebf2; margin: 20px 0;">
    <?php endforeach; ?>
</form>